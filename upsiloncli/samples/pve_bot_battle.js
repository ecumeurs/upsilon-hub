...
while (!gameOver) {
    try {
        // First check: does the current cached state say it's my turn?
        ...
        if (stateResp && stateResp.game_state) {
            ...
            if (board.game_finished) {
                upsilon.log("Game Over detected in initial check.");
                gameOver = true;
                matchId = "";
                break;
            }
            if (board.current_player_is_self) {
                upsilon.log("--- My Turn (Initial/Poll)! Acting with entity: " + board.current_entity_id + " ---");
                executeTacticalLogic(board);
            }
        }

        // Second: Wait for the NEXT update
        ...
        try {
            ...
            if (stateResp && stateResp.game_state) {
                const board = stateResp.game_state;
                if (board.game_finished) {
                    upsilon.log("Game Over detected in poll.");
                    gameOver = true;
                    matchId = "";
                } else if (board.current_player_is_self) {
                    executeTacticalLogic(board);
                }
            }
        } catch (pollErr) {
...
function executeTacticalLogic(board) {
    ...
    if (!actingEntity) {
        upsilon.log("[ERROR] Current entity " + board.current_entity_id + " not found on board.");
        return;
    }

    if (!actingEntity.is_self) {
        upsilon.log("[WARNING] It is my player turn, but not my entity turn (" + actingEntity.name + "). Skipping logic.");
        return;
    }
